	...
	if test.expectedCode == http.StatusOK {
		...
		if test.method == http.MethodPut {
			...
			if !(time.Time(route.UpdatedAt)).Before(time.Now().Add(-1 * time.Hour)) {
				t.Log(buf.String())
				t.Errorf("Test %d: expected updated_at to be set on route, it wasn't: %s", i, route.CreatedAt)
			}
		}
	}
...
func TestRouteCreate(t *testing.T) {
	...
	for i, test := range []funcTestCase{
		// errors
		{commonDS, logs.NewMock(), http.MethodPut, "/v1/funcs/a", ``, http.StatusBadRequest, models.ErrInvalidJSON},
		{commonDS, logs.NewMock(), http.MethodPut, "/v1/funcs/a", `{ }`, http.StatusBadRequest, models.ErrRoutesMissingNew},
		{commonDS, logs.NewMock(), http.MethodPut, "/v1/funcs/a", `{ "image": "yo" }`, http.StatusBadRequest, models.ErrRoutesMissingNew},
		{commonDS, logs.NewMock(), http.MethodPut, "/v1/funcs/a", `{ "func": { } }`, http.StatusBadRequest, models.ErrRoutesMissingImage},
		{commonDS, logs.NewMock(), http.MethodPut, "/v1/funcs/a", `{ "func": { "image": "fnproject/fn-test-utils" } }`, http.StatusBadRequest, models.ErrFuncsMissingVersion},
		{commonDS, logs.NewMock(), http.MethodPut, "/v1/funcs/a", `{ "func": { "image": "fnproject/fn-test-utils", "name": "&%$", "version":"0.0.1" } }`, http.StatusBadRequest, models.ErrFuncsInvalidName},
		{commonDS, logs.NewMock(), http.MethodPut, "/v1/funcs/a", `{ "func": { "image": "fnproject/fn-test-utils", "version": "0.0.1", "format": "wazzup" } }`, http.StatusBadRequest, models.ErrFuncsInvalidFormat},
		{commonDS, logs.NewMock(), http.MethodPut, "/v1/funcs/a", `{ "func": { "image": "fnproject/fn-test-utils", "version": "0.0.1", "cpus": "-100" } }`, http.StatusBadRequest, models.ErrInvalidCPUs},

		// success
		{datastore.NewMock(), logs.NewMock(), http.MethodPut, "/v1/funcs/a", `{ "route": { "image": "fnproject/fn-test-utils", "name": "myfunc", "version":"0.0.1" } }`, http.StatusOK, nil},
		{datastore.NewMock(), logs.NewMock(), http.MethodPut, "/v1/funcs/myfunc", `{ "route": { "image": "fnproject/fn-test-utils", "version": "0.0.2", "cpus": "100m" } }`, http.StatusOK, nil},
		{datastore.NewMock(), logs.NewMock(), http.MethodPut, "/v1/funcs/myfunc", `{ "route": { "image": "fnproject/fn-test-utils", "version": "0.0.3", "cpus": "0.2" } }`, http.StatusOK, nil},
		{datastore.NewMock(), logs.NewMock(), http.MethodPut, "/v1/funcs/myfunc", `{ "route": { "image": "fnproject/fn-test-utils", "version": "0.0.4", "format": "http" } }`, http.StatusOK, nil},
